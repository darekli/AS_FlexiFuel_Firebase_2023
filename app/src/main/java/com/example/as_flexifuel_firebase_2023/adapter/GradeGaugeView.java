...
public class GradeGaugeView extends View {
    ...
    private static final float ANGLE_START = 240f;
    private static final float ANGLE_SWEEP = 120f;
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        if (isInEditMode()) {
            setAdapter(new AdapterLeft());
            setCurrent(20f);
            setLabel("BMI");
        }

        TypedArray a = context.obtainStyledAttributes(attrs,
                com.zhjl37.gaugeview.R.styleable.GradeGaugeView, defStyleAttr, defStyleRes);
...
//        setUseGradient(useGradient);
        ...
        cursorDrawable = ContextCompat.getDrawable(context, R.drawable.ic_circle_mini12);

        cursorPadding = dp2px(0);
        cursorPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        cursorPaint.setTextAlign(Align.CENTER);

        //text l/100km
        labelPadding = dp2px(5);
        ...
        valuePaint.setColor(DEFAULT_COLOR);
        valuePaint.setTextSize(dp2px(12));
    }
    ...
    private void drawLabelAndValue(Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(label)) {
            ...
            canvas.drawText(currentStr, 0, textLength, 0, 0, textPaint);

            textPaint.getTextBounds(currentStr.toString(), 0, textLength, valueRect);

            int labelLength = label.length();
            float labelY = -(valueRect.height() + labelPadding);
            canvas.drawText(label, 0, labelLength, 0, labelY, valuePaint);
        }
    ...
    public static class AdapterLeft extends Adapter {
...
//                Color.parseColor("#ff6e76")
    ...
    public static class AdapterRight extends Adapter {
        private static final String[] TITLES = {"low", "mid", "high"};
        private static final float[][] SCALES = {
                {0f, 6.0f}, {6.0f, 8f}, {8f, 9.5f}
        };
